...
void convertPathsToC(
    double**& cPaths, int& cNumPaths, int*& cPathSizes, const PolygonSet& paths, bool includeDummyZ = false);

// Convert paths to C format
void convertPathsToC(
    double**& cPaths, int& cNumPaths, int*& cPathSizes, const std::vector<std::vector<PointWithZ>>& paths);

// Convert paths from C format
PolygonSet convertPathsFromC(double** paths, int numPaths, int* pathSizes);
}

namespace boost {
namespace polygon {
    template <>
    struct geometry_concept<cam::Polygon> {
    ...
    struct polygon_mutable_traits<cam::Polygon> {
        ...
        static inline cam::Polygon& set_points(cam::Polygon& t, iT input_begin, iT input_end)
        {
            t.clear();

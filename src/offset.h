...
static Polygon rawOffset( //
    const Polygon& path, UnitFromPolygon_t<Polygon> amount, UnitFromPolygon_t<Polygon> arcTolerance, bool closed)
{
    ...
    auto processSegment = [arcTolerance](Polygon& raw, const Point& p0, const Point& p1, const Point& p2, int amount) {
        ...
        auto o = orientation(
            Segment{ p1, { x(p1) + x(normal01), y(p1) + y(normal01) } },
            Segment{ p1, { x(p1) + x(normal12), y(p1) + y(normal12) } });
        if (amount < 0)
            o = -o;
...
static PolygonSet rawOffsetPolygonSet(
    const PolygonSet& ps, UnitFromPolygonSet_t<PolygonSet> amount, UnitFromPolygonSet_t<PolygonSet> arcTolerance,
    bool closed)
{
    PolygonSet result;
    for (auto& poly : ps) {
    ...
    bool closed)
{
    ...
    printf(
        "offset clean time: %d\n",
        (int)std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::high_resolution_clock::now() - cleanStartTime)
            .count());
    printf("polys: %d\n", result.size());

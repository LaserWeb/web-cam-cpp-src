    ...
    for (auto& e : filterEdges) {
        ...
        if (edge.is_linear()) {
            ...
            getCorner(segment1, segment2, [&keep](Point center, Point e1, Point e2) {
                double c
                    = dot(e1 - center, e2 - center) / euclidean_distance(e1, center) / euclidean_distance(e2, center);
                if (c <= cos(95 / 2 / M_PI))
                    keep = false;
            });
            if (!keep)
            ...
            if (cell->source_category() == bp::SOURCE_CATEGORY_SEGMENT_START_POINT
                || cell->source_category() == bp::SOURCE_CATEGORY_SEGMENT_END_POINT) {
                PointWithZ ref;
                if (cell->source_category() == bp::SOURCE_CATEGORY_SEGMENT_START_POINT)
                ...
                for (int i = 0; i <= numSegments; ++i) {
                    PointWithZ p{ lround(x(p1) + (double)i * (x(p2) - x(p1)) / numSegments),
...
void reorderEdges(int debugArg0, int debugArg1, vector<Edge>& edges, Callback callback)
{
    ...
    auto start = find_if(
        edgeIndexes.begin(), edgeIndexes.end(), [](const typename Edge::Index& index) { return !index.point.z; });
    ...
    while (numProcessed < edges.size()) {
        ...
        auto setClosest = [&](typename vector<typename Edge::Index>::iterator it) -> bool {
            if (!it->taken) {
                ...
                if (r > closestRank
                    || r == closestRank
                        && (it->point == p && zDist < closestZdist
                            || (it->point != p || zDist == closestZdist)
                                && (it->point == p && otherZdist < closestOtherZdist
                                    || (it->point != p || otherZdist == closestOtherZdist) && dist < closestDist))) {
    ...
    while (true) {
        ...
        if (reverse) {
            for (auto it = span.rbegin(); it != span.rend(); ++it) {
                ...
                clipTopZ(
                    passDepth, maxDepth, path,
                    Edge{ PointWithZ{ it->point2.x, it->point2.y, it->point2.z + deltaZ },
            ...
            for (auto& edge : span) {
                ...
                clipTopZ(
                    passDepth, maxDepth, path,
                    Edge{ PointWithZ{ edge.point1.x, edge.point1.y, edge.point1.z + deltaZ },
